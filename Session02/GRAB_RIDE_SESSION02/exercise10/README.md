...
Bước 2 (Surge Surcharge): Áp dụng hệ số phụ phí cao điểm độc lập ở bước cuối. // Guard Clauses: Chặn lỗi sớm if (distance_km <= 0.0 || (is_surge != 0 && is_surge != 1)) { return 0; }

// Bước 1: Tính cước cơ bản if (distance_km <= 2.0) { base_fare = 12000.0; } else { base_fare = 12000.0 + (distance_km - 2.0) * 4500.0; }

// Bước 2: Nhân hệ số phụ phí nếu có if (is_surge == 1) { total_fare = base_fare * 1.2; } else { total_fare = base_fare; }
